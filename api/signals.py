...
DEFAULT_THRESHOLDS = {
    ...
    "bb_breakout_margin_pct": 0.5,
    "bb_pct_below_high": 1.0,
    "bb_pct_above_low": 1.0,
    ...
    "donchian_on": 0,
    "donchian_trigger": "breakout",  # breakout | bullish | bearish | middle_cross | two_channel_bull |
                                      # two_channel_bear | resistance_retest | support_retest
    "donchian_mid_cross_lookback": 5,
    "donchian_retest_lookback": 10,
    "donchian_retest_tolerance_pct": 1.0,
    "hma_on": 0,
...
def _active_modes(t: dict, key: str, default: str) -> list:
    """Normalize a trigger setting to a list of active modes — a single indicator
    can now have several trigger modes active at once (each appends its own
    signal), rather than the single mutually-exclusive mode this used to be."""
...
def score_signals(indicators: dict, thresholds: dict | None = None) -> dict:
    ...
    rsi_modes = _active_modes(t, "rsi_trigger", "overbought_oversold")
    if t.get("rsi_on", 1) and rsi is not None:
        if "overbought" in rsi_modes:
        ...
        if "centerline_cross" in rsi_modes:
        ...
        if "bullish_divergence" in rsi_modes:
        ...
        if "bearish_divergence" in rsi_modes:
        ...
        if "failure_swings" in rsi_modes:
        ...
        if "overbought_oversold" in rsi_modes:
            if rsi < t["rsi_oversold"]:
                signals.append({"indicator": "RSI", "type": "BUY", "detail": f"RSI {rsi:.1f} — oversold (<{t['rsi_oversold']})", "weight": 2})
                buy_score += 2
    ...
    macd_modes = _active_modes(t, "macd_trigger", "signal_cross")
    if t.get("macd_on", 1) and macd_val is not None and macd_sig_val is not None:
        ...
        if "bullish_signal_cross" in macd_modes:
        ...
        if "bearish_signal_cross" in macd_modes:
            if macd_crossed_recently and macd_dir < 0:
        ...
        if "centerline_cross" in macd_modes:
        ...
        if "bullish_divergence" in macd_modes:
        ...
        if "bearish_divergence" in macd_modes:
        ...
        if "histogram_reversal" in macd_modes:
    ...
    bb_modes = _active_modes(t, "bb_trigger", "percent_b")
    if t.get("bb_on", 1) and pct_b is not None:
        if "upper_touch" in bb_modes:
        ...
        if "volatility_breakout" in bb_modes:
        ...
        if "breakout_margin" in bb_modes:
            upper, lower = bb.get("upper"), bb.get("lower")
            margin = t.get("bb_breakout_margin_pct", 0.5) / 100
            if upper is not None and close is not None and close >= upper * (1 + margin):
                signals.append({"indicator": "Bollinger Bands", "type": "BUY", "detail": f"Closed beyond the upper band by more than {t.get('bb_breakout_margin_pct', 0.5)}%", "weight": 1})
                buy_score += 1
            elif lower is not None and close is not None and close <= lower * (1 - margin):
                signals.append({"indicator": "Bollinger Bands", "type": "SELL", "detail": f"Closed beyond the lower band by more than {t.get('bb_breakout_margin_pct', 0.5)}%", "weight": 1})
                sell_score += 1
            else:
                signals.append({"indicator": "Bollinger Bands", "type": "NEUTRAL", "detail": "No margin-confirmed breakout", "weight": 0})

        if "pct_below_high" in bb_modes:
            upper = bb.get("upper")
            thresh = t.get("bb_pct_below_high", 1.0)
            if upper is not None and close is not None and close <= upper:
                dist = (upper - close) / upper * 100
                if dist <= thresh:
                    signals.append({"indicator": "Bollinger Bands", "type": "SELL", "detail": f"Price is within {thresh}% below the upper band — approaching resistance", "weight": 1})
                    sell_score += 1
                else:
                    signals.append({"indicator": "Bollinger Bands", "type": "NEUTRAL", "detail": "Not close enough to the upper band", "weight": 0})
            else:
                signals.append({"indicator": "Bollinger Bands", "type": "NEUTRAL", "detail": "Price above the upper band or unavailable", "weight": 0})

        if "pct_above_low" in bb_modes:
            lower = bb.get("lower")
            thresh = t.get("bb_pct_above_low", 1.0)
            if lower is not None and close is not None and close >= lower:
                dist = (close - lower) / lower * 100
                if dist <= thresh:
                    signals.append({"indicator": "Bollinger Bands", "type": "BUY", "detail": f"Price is within {thresh}% above the lower band — approaching support", "weight": 1})
                    buy_score += 1
                else:
                    signals.append({"indicator": "Bollinger Bands", "type": "NEUTRAL", "detail": "Not close enough to the lower band", "weight": 0})
            else:
                signals.append({"indicator": "Bollinger Bands", "type": "NEUTRAL", "detail": "Price below the lower band or unavailable", "weight": 0})

        if "percent_b" in bb_modes:
    ...
    ma_modes = _active_modes(t, "ma_trigger", "dual_cross")
    ma_trig_lookback = int(t.get("ma_trigger_lookback", 5))
    if t.get("ma_on", 1):
        if "price_cross" in ma_modes:
    ...
    if t.get("adx_on", 0) and adx_v is not None and dmp_v is not None and dmn_v is not None:
        ...
        for _mode_name, fixed_threshold in (("above_25", 25), ("above_50", 50), ("above_75", 75)):
            if _mode_name in adx_modes:
        ...
        if "strong_di_plus" in adx_modes:
        ...
        if "strong_di_minus" in adx_modes:
        ...
        if "trend_threshold" in adx_modes:
    ...
    if t.get("psar_on", 0) and psar.get("is_bull") is not None:
        ...
        if "trailing_stop" in psar_modes:
    ...
    ichimoku_modes = _active_modes(t, "ichimoku_trigger", "cloud_position")
    if t.get("ichimoku_on", 0) and cloud_pos:
        if "bullish" in ichimoku_modes:
            if cloud_pos > 0:
        ...
        if "cloud_position" in ichimoku_modes:
            if cloud_pos > 0:
                signals.append({"indicator": "Ichimoku", "type": "BUY", "detail": "Price above the cloud — bullish", "weight": 2})
                buy_score += 2
            elif cloud_pos < 0:
    ...
    supertrend_modes = _active_modes(t, "supertrend_trigger", "flip")
    if t.get("supertrend_on", 0) and supertrend.get("is_bull") is not None:
        ...
        if "bull_flip" in supertrend_modes:
        ...
        if "bear_flip" in supertrend_modes:
        ...
        if "trend_state" in supertrend_modes:
        ...
        if "trailing_stop" in supertrend_modes:
    ...
    donchian_modes = _active_modes(t, "donchian_trigger", "breakout")
    if t.get("donchian_on", 0) and donchian.get("upper") is not None:
        close_v = donchian.get("close")
        if "bullish" in donchian_modes:
        ...
        if "middle_cross" in donchian_modes:
        ...
        if "two_channel_bull" in donchian_modes:
        ...
        if "two_channel_bear" in donchian_modes:
            ...
            if close_v is not None and close_v <= donchian["lower"]:
                ...
                signals.append({"indicator": "Donchian", "type": "NEUTRAL", "detail": "No entry or exit breakout", "weight": 0})
        if "resistance_retest" in donchian_modes:
            bars = donchian.get("upper_cross_bars_since", 999)
            upper = donchian.get("upper")
            tol = t.get("donchian_retest_tolerance_pct", 1.0)
            lookback = int(t.get("donchian_retest_lookback", 10))
            if bars <= lookback and upper and close_v is not None and close_v >= upper * (1 - tol / 100):
                signals.append({"indicator": "Donchian", "type": "BUY", "detail": f"Resistance retest — price pulled back to the former breakout level within {tol}% and is holding", "weight": 1})
                buy_score += 1
            else:
                signals.append({"indicator": "Donchian", "type": "NEUTRAL", "detail": "No resistance retest", "weight": 0})
        if "support_retest" in donchian_modes:
            bars = donchian.get("lower_cross_bars_since", 999)
            lower = donchian.get("lower")
            tol = t.get("donchian_retest_tolerance_pct", 1.0)
            lookback = int(t.get("donchian_retest_lookback", 10))
            if bars <= lookback and lower and close_v is not None and close_v <= lower * (1 + tol / 100):
                signals.append({"indicator": "Donchian", "type": "SELL", "detail": f"Support retest — price pulled back to the former breakdown level within {tol}% and is holding", "weight": 1})
                sell_score += 1
            else:
                signals.append({"indicator": "Donchian", "type": "NEUTRAL", "detail": "No support retest", "weight": 0})
        if "breakout" in donchian_modes:
            if close_v is not None and close_v >= donchian["upper"]:
                signals.append({"indicator": "Donchian", "type": "BUY", "detail": "Breakout above the upper channel", "weight": 1})
                buy_score += 1
    ...
    stoch_k = stoch.get("k")
    stoch_modes = _active_modes(t, "stoch_trigger", "overbought_oversold")
    if t.get("stoch_on", 0) and stoch_k is not None:
        if "overbought" in stoch_modes:
        ...
        if "signal_cross" in stoch_modes:
        ...
        if "overbought_oversold" in stoch_modes:
            if stoch_k < t["stoch_oversold"]:
                signals.append({"indicator": "Stochastic", "type": "BUY", "detail": f"%K {stoch_k:.1f} — oversold", "weight": 1})
                buy_score += 1
    ...
    stochrsi_modes = _active_modes(t, "stochrsi_trigger", "overbought_oversold")
    if t.get("stochrsi_on", 0) and srsi_k is not None:
        if "overbought" in stochrsi_modes:
        ...
        if "signal_cross" in stochrsi_modes:
        ...
        if "bullish_divergence" in stochrsi_modes:
        ...
        if "bearish_divergence" in stochrsi_modes:
        ...
        if "overbought_oversold" in stochrsi_modes:
            if srsi_k < t["stochrsi_oversold"]:
                signals.append({"indicator": "Stochastic RSI", "type": "BUY", "detail": f"%K {srsi_k:.1f} — oversold", "weight": 1})
                buy_score += 1
    ...
    if t.get("cci_on", 0) and cci_v is not None:
        ...
        if "centerline_cross" in cci_modes:
        ...
        if "overbought_oversold" in cci_modes:
            if cci_v < t["cci_oversold"]:
                signals.append({"indicator": "CCI", "type": "BUY", "detail": f"CCI {cci_v:.1f} — oversold", "weight": 1})
                buy_score += 1
    ...
    if t.get("willr_on", 0) and willr_v is not None:
        ...
        if "midline_cross" in willr_modes:
        ...
        if "momentum_failure_bull" in willr_modes:
        ...
        if "momentum_failure_bear" in willr_modes:
        ...
        if "trend_confirmation_bull" in willr_modes:
        ...
        if "trend_confirmation_bear" in willr_modes:
        ...
        if "bullish_divergence" in willr_modes:
        ...
        if "bearish_divergence" in willr_modes:
        ...
        if "overbought_oversold" in willr_modes:
            if willr_v < t["willr_oversold"]:
                signals.append({"indicator": "Williams %R", "type": "BUY", "detail": f"%R {willr_v:.1f} — oversold", "weight": 1})
                buy_score += 1
    ...
    if t.get("roc_on", 0) and roc_v is not None:
        ...
        if "centerline_cross" in roc_modes:
        ...
        if "bullish_divergence" in roc_modes:
        ...
        if "bearish_divergence" in roc_modes:
    ...
    if t.get("mfi_on", 0) and mfi_v is not None:
        ...
        if "centerline_cross" in mfi_modes:
        ...
        if "bullish_divergence" in mfi_modes:
        ...
        if "bearish_divergence" in mfi_modes:
        ...
        if "overbought_oversold" in mfi_modes:
            if mfi_v < t["mfi_oversold"]:
                signals.append({"indicator": "MFI", "type": "BUY", "detail": f"MFI {mfi_v:.1f} — oversold", "weight": 1})
                buy_score += 1
    ...
    tsi_modes = _active_modes(t, "tsi_trigger", "signal_cross")
    if t.get("tsi_on", 0) and tsi_v is not None and tsi_sig is not None:
        if "bullish" in tsi_modes:
            if tsi_v > tsi_sig:
        ...
        if "centerline_cross" in tsi_modes:
        ...
        if "bullish_divergence" in tsi_modes:
        ...
        if "bearish_divergence" in tsi_modes:
    ...
    ao_v = ao.get("value")
    ao_modes = _active_modes(t, "ao_trigger", "zero_state")
    if t.get("ao_on", 0) and ao_v is not None:
        if "bullish" in ao_modes:
            if ao_v > 0:
        ...
        if "bull_twin_peaks" in ao_modes:
        ...
        if "bear_twin_peaks" in ao_modes:
        ...
        if "bull_divergence" in ao_modes:
        ...
        if "bear_divergence" in ao_modes:
    ...
    def _vol_direction_signal(name, key, expanding, close_v, close_prev):
        ...
        modes = _active_modes(t, f"{key}_trigger", "expansion")
        rising = close_v > close_prev
        falling = close_v < close_prev
        if "bullish_expansion" in modes:
    ...
    keltner_modes = _active_modes(t, "keltner_trigger", "breakout")
    if t.get("keltner_on", 0) and keltner.get("upper") is not None and close_now is not None:
        if "bullish" in keltner_modes:
        ...
        if "middle_cross" in keltner_modes:
        ...
        if "bull_band_riding" in keltner_modes:
        ...
        if "bear_band_riding" in keltner_modes:
        ...
        if "bull_mean_reversion" in keltner_modes:
        ...
        if "bear_mean_reversion" in keltner_modes:
            if close_now >= keltner["upper"]:
                ...
                signals.append({"indicator": "Keltner Channels", "type": "NEUTRAL", "detail": "No mean-reversion setup", "weight": 0})
        if "keltner_squeeze" in keltner_modes:
    ...
    if t.get("vwap_on", 0) and vwap_v is not None and close_now is not None:
        ...
        if "pullback_sell" in vwap_modes:
            prev_close, prev_value = vwap.get("prev_close"), vwap.get("prev_value")
            if (prev_close is not None and prev_value is not None
                    and prev_close >= prev_value and close_now < vwap_v):
    ...
    cmf_v = cmf.get("value")
    cmf_modes = _active_modes(t, "cmf_trigger", "threshold")
    if t.get("cmf_on", 0) and cmf_v is not None:
        if "bullish" in cmf_modes:
        ...
        if "centerline_cross" in cmf_modes:
    ...
    vol_profile_modes = _active_modes(t, "vol_profile_trigger", "position")
    if t.get("vol_profile_on", 0) and vp.get("poc") is not None and close_now is not None:
        if "bullish" in vol_profile_modes:
            if close_now > vp["poc"]:
        ...
        if "bearish" in vol_profile_modes:
            if close_now < vp["poc"]:
        ...
        if "poc_breakout" in vol_profile_modes:
        ...
        if "position" in vol_profile_modes:
            if close_now > vp["poc"]:
                signals.append({"indicator": "Volume Profile", "type": "BUY", "detail": "Price above the point of control", "weight": 1})
                buy_score += 1
    ...
    fib_modes = _active_modes(t, "fib_trigger", "bounce_reject")
    if (t.get("fib_on", 0) and nearest is not None and dist_pct is not None
        ...
        if "bullish_bounce" in fib_modes:
        ...
        if "bearish_reject" in fib_modes:
